<script setup>
  ...
  const emits = defineEmits([
      'confirmar-borrado-cliente',
      'confirmar-reseteo-clientes'
  ])
  ...
  const handleConfirm = () => {
    if (props.modal.removeAll) {
      emits('confirmar-reseteo-clientes')
    } else {
      emits('confirmar-borrado-cliente')
    }
    closeModal()
  }
</script>
    ...
    <div class="relative top-40 mx-auto shadow-xl rounded-md max-w-md">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-4 md:p-5 text-center">
          ...
          <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{ modal.message }}</h3>
          <button
              data-modal-hide="popup-modal"
              type="button"
